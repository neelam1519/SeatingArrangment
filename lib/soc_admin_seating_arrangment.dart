  ...
  class _SocAdminSeatingArrangementState extends State<SocAdminSeatingArrangement> {
    GlobalKey<ProgressDialogState> progressDialogKey = GlobalKey<ProgressDialogState>();
        ...
        body: SingleChildScrollView(
          child: Column(
            children: [
              ...
              SizedBox(height: 20), // Add some space between Buttons and TextView + Button
              Column(
                children: batchesMap.keys
                    ...
                    children: [
                      Padding(
                        padding: EdgeInsets.only(left: 20), // Add space to the left
                        child: Text('$batch: ${batchesMap[batch]!.join(', ')}'),
                      ),
                      Padding(
                        padding: EdgeInsets.only(right: 20), // Add space to the right
    ...
    void _ArrangeStudentsSeating() async {
      ...
      List<List<dynamic>> classrooms = await readCsv(uploadedFiles["HallNumbers"]!);
      List<List<dynamic>> lecturers = await readCsv(uploadedFiles["LecturerNames"]!);

      await Future.forEach(batchesMap.entries, (MapEntry<String, List<String>> entry) async {
    ...
    Future<void> _assignStudentsToClassrooms(List<List<dynamic>> batch1, List<List<dynamic>> batch2, List<List<dynamic>> batch3, List<List<dynamic>> classrooms,List<List<dynamic>> lecurernames) async {
      int batch1Counter = 0;
      int batch2Counter = 0;
      int batch3Counter = 0;
      ...
      try {
        ...
        for (int i = 0; i < classrooms.length; i++) {
          ...
          await uploadLecturerName("SOC_FILES", widget.documentname, blockNumber, classNumber, "LecturerName", lecturerName);
          ...
          for (int j = 0; j < classroomCapacity; j++) {
            ...
            if (j % 3 == 0) {
              ...
              if (batch1Counter < batch1.length) {
                ...
                await uploadData("SOC_FILES", widget.documentname, blockNumber, classNumber, batch1[batch1Counter][0].toString(), batch1[batch1Counter],);
                batch1Counter++;
              }
            } else if (j % 3 == 1) {
              currentIteration++;
              if (batch2Counter < batch2.length) {
                ...
                await uploadData("SOC_FILES", widget.documentname, blockNumber, classNumber, batch2[batch2Counter][0].toString(), batch2[batch2Counter],);
                batch2Counter++;
              }
            } else {
              currentIteration++;
              if (batch3Counter < batch3.length) {
                ...
                await uploadData("SOC_FILES", widget.documentname, blockNumber, classNumber, batch3[batch3Counter][0].toString(), batch3[batch3Counter],);
                batch3Counter++;
              }
            }
            // Update progress
    ...
    Future<void> uploadData(String collectionName, String documentName, String subcollectionName, String subdocumentName, String fieldName, List<dynamic> value) async {
      ...
      CollectionReference subcollection = document.collection(subcollectionName.toString());
      DocumentReference subdocument = subcollection.doc(subdocumentName.toString());
    ...
    Future<void> uploadLecturerName(String collectionName, String documentName, String subcollectionName, String subdocumentName, String fieldName, String value) async {
      // Reference to Firestore collection, document, subcollection, and subdocument
      CollectionReference collection = FirebaseFirestore.instance.collection(collectionName);
      DocumentReference document = collection.doc(documentName);
      CollectionReference subcollection = document.collection(subcollectionName.toString());
    ...
    void _splitInto2(List<List<dynamic>> all, List<List<dynamic>> batch1, List<List<dynamic>> batch3) {
    ...
    void _splitInto3(List<List<dynamic>> all, List<List<dynamic>> batch1, List<List<dynamic>> batch2, List<List<dynamic>> batch3) {
      int totalRows = all.length;
    ...
    Future<List<List<dynamic>>> readCsv(File file) async {
      try {
        List<List<dynamic>> csvList = CsvToListConverter().convert(await file.readAsString());
    ...
    void _uploadFilesToStorage() async {
      try {
        ...
        for (MapEntry<String, File> entry in uploadedFiles.entries) {
          ...
          final uploadTask =
          storageRef.child("SOC_FILES/${widget.documentname}/$fileName").putFile(file);
    ...
    void _uploadFileLinksToFirestore() async {
      try {
        ...
        for (MapEntry<String, String> entry in uploadedFilesLinks.entries) {
          ...
          print('Firestore updated successfully for batch $batch with link: $downloadURL');
        }
    ...
    void fetchDataFromFirestore() async {
      try {
        ...
        await firestore.collection('SOC_FILES').doc(widget.documentname).get();
    ...
    Future<File?> downloadFile(String url) async {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          final parentDirectoryExists = await Directory(parentDirectoryPath).exists();
  ...
  class FileUploadButton extends StatelessWidget {
    ...
    const FileUploadButton({required this.documentName, required this.batchNumber, required this.uploadedFiles});

    Future<void> _handleFileUpload() async { // Remove underscore to make it public
    ...
    Future<void> _saveFileInternally(String documentName, String fileName, File file) async {
      try {
        final directory = await getApplicationSupportDirectory();
  ...
  class ProgressDialog extends StatefulWidget {
    ...
    void updateProgress(double newProgress) {
      (key as GlobalKey<ProgressDialogState>).currentState?.updateProgress(newProgress);
    }
  }
